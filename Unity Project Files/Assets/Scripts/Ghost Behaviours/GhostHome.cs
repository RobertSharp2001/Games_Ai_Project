...
public class GhostHome : GhostBehaviour{
    ...
    private IEnumerator ExitTransition(){
        ...
        while (elapsed < duration){
            ghost.SetPosition(Vector3.Lerp(position, ghost.inside.position, elapsed / duration));
        ...
        while (elapsed < duration)
        {
            ghost.SetPosition(Vector3.Lerp(ghost.inside.position, ghost.outside.position, elapsed / duration));
            elapsed += Time.deltaTime;
            yield return null;
        }
